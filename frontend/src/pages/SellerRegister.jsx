...
import SellerLoginScene from "@/components/SellerLoginScene";
...
const SellerRegister = () => {
  const [form, setForm] = useState({ name: "", email: "", phone: "", farmLocation: "", password: "", confirmPassword: "" });
  ...
  const fields = [
    ...
    { key: "farmLocation", icon: MapPin, placeholder: "Farm Location", type: "text" },
  ...
  return (
    <div className="min-h-screen flex overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative items-end justify-center overflow-hidden" style={{ background: "var(--gradient-auth-bg)" }}>
        <div className="absolute inset-0"><SellerLoginScene /></div>
        <div className="relative z-10 text-center px-10 pb-12">
          <h2 className="text-3xl font-display font-bold text-white mb-2">Become a Seller</h2>
          <p className="text-white/60 text-sm max-w-xs mx-auto leading-relaxed">
            List your grain stock and reach buyers directly. No middlemen, fair prices.
          </p>
        </div>
      </div>
      <div className="flex-1 relative flex items-center justify-center px-8 py-8 bg-background overflow-y-auto overflow-x-hidden">
        ...
        <div className="relative z-10 w-full max-w-md">
          <h1 className="text-3xl font-display font-bold text-foreground mb-2">Seller Registration</h1>
          <p className="text-muted-foreground mb-6">Join Uzhavan as a farmer</p>
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            Already have an account?{" "}<Link to="/seller/login" className="text-primary font-semibold hover:underline">Login</Link>
          </p>
        </div>
      </div>
    </div>
