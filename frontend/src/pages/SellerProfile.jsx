...
const SellerProfile = () => {
  ...
  const userData = user || {};
  const initial = (userData.name || "F").charAt(0).toUpperCase();

  const handleSave = async () => {
    setSaving(true);
    try {
      await updateProfile({ name: form.name, phone: form.phone, farmLocation: form.farmLocation });
      setEditing(false);
  ...
  const editFields = [
    { key: "name", icon: User, label: "Farmer Name" },
    { key: "phone", icon: Phone, label: "Phone" },
    { key: "farmLocation", icon: MapPin, label: "Farm Location" },
  ];

  const viewFields = [
    { icon: User, label: "Farmer Name", value: userData.name },
    { icon: Phone, label: "Phone", value: userData.phone },
    { icon: MapPin, label: "Farm Location", value: userData.farmLocation },
        ...
        <div className="relative z-10 pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-2xl mx-auto">
          <div className="bg-card rounded-2xl border border-border overflow-hidden" style={{ boxShadow: "var(--shadow-elevated)" }}>
            <div className="flex flex-col items-center pt-10 pb-6 px-8" style={{ background: "var(--gradient-hero)" }}>
              ...
              <h1 className="text-2xl font-display font-bold text-foreground">{userData.name || "Farmer"}</h1>
              <p className="text-muted-foreground text-sm mt-1">Seller Account 🌾</p>
            </div>
